...
def propagate_conv_linear(relevant, irrelevant, module):
    ...
    # total proportion
    #     prop_rel = torch.sum(torch.abs(relevant))
    #     prop_irrel = torch.sum(torch.abs(irrelevant))
    ...
    #     prop_irrel = 0.5
    #     prop_rel = 1
    #     return rel - prop_irrel * bias, irrel - prop_rel * bias
...
def propagate_activation(relevant, irrelevant, activation):
    swap_inplace = False
    ...
    # rel_score = 0.5 * (activation(relevant) - activation(zeros) + activation(relevant + irrelevant) - activation(irrelevant))
    # irrel_score = 0.5 * (activation(irrelevant) - activation(zeros) + activation(relevant + irrelevant) - activation(relevant))
    if swap_inplace:
        activation.inplace = True
    return rel_score, irrel_score


def propagate_pooling(relevant, irrelevant, pooler, model_type='alexnet'):
    if model_type == 'mnist':
        unpool = torch.nn.MaxUnpool2d(kernel_size=2, stride=2)
        avg_pooler = torch.nn.AvgPool2d(kernel_size=2, stride=2)
        window_size = 4
    elif model_type == 'alexnet':
        unpool = torch.nn.MaxUnpool2d(kernel_size=3, stride=2)
        avg_pooler = torch.nn.MaxPool2d(kernel_size=3, stride=2)
        window_size = 1
    elif model_type == 'vgg':
        unpool = torch.nn.MaxUnpool2d(kernel_size=2, stride=2)
        avg_pooler = torch.nn.AvgPool2d(kernel_size=(2, 2), stride=(2, 2), count_include_pad=False)
        window_size = 4
    ...
    #     in_min = torch.min(input_rel)
    #     input_rel -= in_min # set min to 0
    #     print('in', input_rel[0, 0, :25, :10])
    rel = avg_pooler(rel) * window_size
    #     print('in_min', in_min)
    #     rel += in_min # reset min to correct value
    #     print('out', rel[0, 0, :10, :10])

    # irrelevant
    irrel = mask_both * irrelevant
    #     in_min = torch.min(input_irrel)
    #     input_irrel -= in_min
    irrel = avg_pooler(irrel) * window_size
    #     irrel += in_min

    # analyze things with problems
    #     rel, irrel =
    # problem - there are multiple things - only want to select one thing per pool but things overlap because stride < kernel_size
    #     print("____________________IN______________________")
    #     print('rel', relevant[0, 0, :10, :10])
    #     print('irrel', irrelevant[0, 0, :10, :10])
    #     print('both', (relevant + irrelevant)[0, 0, :10, :10])
    #     print("____________________OUT______________________")
    #     print('rel', rel[0, 0, :10, :10])
    #     print('irrel', irrel[0, 0, :10, :10])
    #     print('both', both[0, 0, :10, :10])
    #     print('diff', (both - (rel + irrel))[0, 0, :10, :10])
    #     print(rel.shape, both.shape)
    #     return both - irrel, irrel
    return rel, irrel


#     return 0.5 * (pooler(relevant + irrelevant) - pooler(irrelevant) + pooler(relevant)), 0.5 * (pooler(relevant+ irrelevant) - pooler(relevant) + pooler(irrelevant)) # split importances
...
def cd(blob, im_torch, model, model_type='mnist'):
    ...
    if model_type == 'mnist':
        ...
        relevant, irrelevant = propagate_activation(relevant, irrelevant, F.relu)
    ...
    # alexnet 
        ...
        #         nn.MaxPool2d(kernel_size=3, stride=2),
        relevant, irrelevant = propagate_conv_linear(relevant, irrelevant, mods[1])
        relevant, irrelevant = propagate_activation(relevant, irrelevant, mods[2])
        relevant, irrelevant = propagate_pooling(relevant, irrelevant, mods[3], model_type=model_type)

        relevant, irrelevant = propagate_conv_linear(relevant, irrelevant, mods[4])
        relevant, irrelevant = propagate_activation(relevant, irrelevant, mods[5])
        relevant, irrelevant = propagate_pooling(relevant, irrelevant, mods[6], model_type=model_type)

        relevant, irrelevant = propagate_conv_linear(relevant, irrelevant, mods[7])
        relevant, irrelevant = propagate_activation(relevant, irrelevant, mods[8])

        relevant, irrelevant = propagate_conv_linear(relevant, irrelevant, mods[9])
        relevant, irrelevant = propagate_activation(relevant, irrelevant, mods[10])

        relevant, irrelevant = propagate_conv_linear(relevant, irrelevant, mods[11])
        relevant, irrelevant = propagate_activation(relevant, irrelevant, mods[12])
        relevant, irrelevant = propagate_pooling(relevant, irrelevant, mods[13], model_type=model_type)

        # relevant = relevant.view(relevant.size(0), 256 * 6 * 6)
        # irrelevant = irrelevant.view(irrelevant.size(0), 256 * 6 * 6)

        relevant = relevant.view(-1, 256 * 6 * 6)
        irrelevant = irrelevant.view(-1, 256 * 6 * 6)
        # classifier
        #         nn.Dropout(),
        ...
        relevant, irrelevant = propagate_dropout(relevant, irrelevant, mods[15])
        relevant, irrelevant = propagate_conv_linear(relevant, irrelevant, mods[16])
        relevant, irrelevant = propagate_activation(relevant, irrelevant, mods[17])

        relevant, irrelevant = propagate_dropout(relevant, irrelevant, mods[18])
        relevant, irrelevant = propagate_conv_linear(relevant, irrelevant, mods[19])
        relevant, irrelevant = propagate_activation(relevant, irrelevant, mods[20])

        relevant, irrelevant = propagate_conv_linear(relevant, irrelevant, mods[21])

        # note: alexnet contains no softmax
    elif model_type == 'vgg':
        ...
        relevant, irrelevant = propagate_activation(relevant, irrelevant, mods[1])
        relevant, irrelevant = propagate_conv_linear(relevant, irrelevant, mods[2])
        scores.append((relevant.clone(), irrelevant.clone()))
        relevant, irrelevant = propagate_activation(relevant, irrelevant, mods[3])
        ...
        scores.append((relevant.clone(), irrelevant.clone()))
        relevant, irrelevant = propagate_activation(relevant, irrelevant, mods[6])
        relevant, irrelevant = propagate_conv_linear(relevant, irrelevant, mods[7])
        scores.append((relevant.clone(), irrelevant.clone()))
        relevant, irrelevant = propagate_activation(relevant, irrelevant, mods[8])
        ...
        scores.append((relevant.clone(), irrelevant.clone()))
        relevant, irrelevant = propagate_activation(relevant, irrelevant, mods[11])
        relevant, irrelevant = propagate_conv_linear(relevant, irrelevant, mods[12])
        scores.append((relevant.clone(), irrelevant.clone()))
        relevant, irrelevant = propagate_activation(relevant, irrelevant, mods[13])
        relevant, irrelevant = propagate_conv_linear(relevant, irrelevant, mods[14])
        scores.append((relevant.clone(), irrelevant.clone()))
        relevant, irrelevant = propagate_activation(relevant, irrelevant, mods[15])
        ...
        scores.append((relevant.clone(), irrelevant.clone()))
        relevant, irrelevant = propagate_activation(relevant, irrelevant, mods[18])
        relevant, irrelevant = propagate_conv_linear(relevant, irrelevant, mods[19])
        scores.append((relevant.clone(), irrelevant.clone()))
        relevant, irrelevant = propagate_activation(relevant, irrelevant, mods[20])
        relevant, irrelevant = propagate_conv_linear(relevant, irrelevant, mods[21])
        scores.append((relevant.clone(), irrelevant.clone()))
        relevant, irrelevant = propagate_activation(relevant, irrelevant, mods[22])
        ...
        scores.append((relevant.clone(), irrelevant.clone()))
        relevant, irrelevant = propagate_activation(relevant, irrelevant, mods[25])
        relevant, irrelevant = propagate_conv_linear(relevant, irrelevant, mods[26])
        scores.append((relevant.clone(), irrelevant.clone()))
        relevant, irrelevant = propagate_activation(relevant, irrelevant, mods[27])
        relevant, irrelevant = propagate_conv_linear(relevant, irrelevant, mods[28])
        scores.append((relevant.clone(), irrelevant.clone()))
        relevant, irrelevant = propagate_activation(relevant, irrelevant, mods[29])
        ...
        relevant, irrelevant = propagate_activation(relevant, irrelevant, mods[33])
        ...
        relevant, irrelevant = propagate_activation(relevant, irrelevant, mods[36])
